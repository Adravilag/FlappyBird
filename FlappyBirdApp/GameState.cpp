...
namespace Sonar
{
	...
	void GameState::Init()
	{
		...
		if (!_playingMusic.openFromFile(PLAYING_MUSIC_FILEPATH))
		{
			std::cout << "Error loading Playing Music" << std::endl;
		}
		...
		this->_data->assets.LoadTexture("Scoring Pipe", PIPE_SCORING_PIPE_FILEPATH);
